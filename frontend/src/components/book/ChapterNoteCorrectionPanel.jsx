        ...
        <div className="noteCorrectionScopeList" aria-label="section list">
          {sections.map((section) => (
            ...
            >
              ...
              {section.has_pn68yptt && <em className="noteCorrectionPn68Badge">SYNPN068 warning</em>}
            </button>
          ))}
        </div>
      )}
          ...
          <div className="noteCorrectionScopeList">
            {batches.map((batch) => (
              ...
              >
                ...
                {batch.has_pn68yptt && <em className="noteCorrectionPn68Badge">SYNPN068 warning</em>}
              </button>
            ))}
          </div>
        </div>
...
export function buildNoteCorrectionCopyPrompt(payload) {
  ...
  return [
    ...
    "SYNPN068 未匹配到 chunk，后续纠错审核需谨慎。",
    "SYNPN068 的 anchor_method 必须保持 unmatched / alignment_uncertain，不得假装已对齐正文。",
    "",
    "## 精简 ChatGPT 输入包",
...
function buildChapterMergePreview(plan, validationStates, chapterId, mode, batchSize) {
  ...
  for (const [key, state] of Object.entries(validationStates || {})) {
    ...
    for (const item of validation.normalized_preview || []) {
      ...
      if (item.zotero_annotation_key === "SYNPN068") pn68Status = item.correction_status || "validated";
    }
  }
