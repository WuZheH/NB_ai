...
@pytest.mark.parametrize(
    "invalid_vector",
    (
        pytest.param([0.125, 0.25], id="wrong_vector_length"),
        pytest.param([0.125, float("inf"), 0.5], id="positive_inf_vector"),
        pytest.param([0.125, float("-inf"), 0.5], id="negative_inf_vector"),
    ),
)
def test_reused_note_scope_reconciliation_rejects_invalid_vector_before_mutation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    database, store, manifest, _row = _reused_scope_candidate(
        tmp_path,
        monkeypatch,
        corrupt_field="vector",
        corrupt_value=invalid_vector,
    )
    db = vector_store_service.open_vector_store(store)
    before_count = db.open_table(vector_store_service.NOTE_TABLE).count_rows()
    before_manifest = manifest.read_bytes()
    ...
    table = db.open_table(vector_store_service.NOTE_TABLE)
    assert table.count_rows() == before_count
    assert table.count_rows("document_id = 1") == 1
    assert manifest.read_bytes() == before_manifest
...
@pytest.mark.parametrize(
    "invalid_vector",
    (
        pytest.param("not-a-vector", id="non_vector_value"),
        pytest.param([0.125, float("nan"), 0.5], id="nan_vector"),
    ),
)
def test_note_metadata_comparison_rejects_invalid_vector_value(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    invalid_vector: object,
) -> None:
    database, _store, _manifest, row = _reused_scope_candidate(
        tmp_path,
        monkeypatch,
    )
    source = vector_store_service.collect_personal_note_sources(
    ...
    row["document_id"] = 2
    row["vector"] = invalid_vector

    assert not vector_store_service._note_record_matches_authoritative_metadata(
        row,
        source,
        embedding_dim=3,
    )
