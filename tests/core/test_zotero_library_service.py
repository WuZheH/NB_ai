...
def _fixture(tmp_path, monkeypatch, titles=("EDSR",)):
    ...
    with sqlite3.connect(snap) as c:
        c.executescript("CREATE TABLE items(itemID INTEGER PRIMARY KEY,key TEXT,itemTypeID INTEGER); CREATE TABLE itemTypes(itemTypeID INTEGER PRIMARY KEY,typeName TEXT); CREATE TABLE fields(fieldID INTEGER PRIMARY KEY,fieldName TEXT); CREATE TABLE itemData(itemID INTEGER,fieldID INTEGER,valueID INTEGER); CREATE TABLE itemDataValues(valueID INTEGER PRIMARY KEY,value TEXT); CREATE TABLE itemAttachments(itemID INTEGER PRIMARY KEY,parentItemID INTEGER,key TEXT,path TEXT,contentType TEXT); CREATE TABLE itemAnnotations(itemID INTEGER,parentItemID INTEGER); CREATE TABLE itemNotes(itemID INTEGER,parentItemID INTEGER);")
        ...
        c.execute("INSERT INTO itemAttachments VALUES(10,1,'ATT1','storage:paper.pdf','application/pdf')"); c.execute("INSERT INTO itemAnnotations VALUES(20,10)"); c.execute("INSERT INTO itemNotes VALUES(30,10)"); c.commit()
