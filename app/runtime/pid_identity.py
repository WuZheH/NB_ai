...
def get_loopback_listener_identity(port: int) -> ProcessIdentity | None:
    """Resolve a loopback listener for diagnostics without taking ownership."""
    ...
    for connection in connections:
        ...
        if (
            ...
            or not address
            or int(address.port) != int(port)
            or str(address.ip) not in {"127.0.0.1", "::1"}
            or not connection.pid
        ):
            continue
        try:
            return get_process_identity(int(connection.pid))
        except ProcessIdentityUnavailable:
            return None
    return None
