    ...
    *,
    ...
    return {
        ...
        "warning_summary": {
            ...
            "pn68yptt": _warning_item(package, "SYNPN068"),
        },
    ...
    *,
    ...
    if review_mode == "section_scoped":
        ...
        scope = {
            ...
            "pn68_in_scope": "SYNPN068" in (section.get("zotero_annotation_keys") or []),
        }
        ...
        scope = {
            ...
            "expected_count": len(scoped_candidates),
            "expected_note_ids": [_candidate_canonical_key(item) for item in scoped_candidates],
            "pn68_in_scope": "SYNPN068" in (batch.get("zotero_annotation_keys") or []),
        }
...
def build_chapter_note_correction_canary_subscope_package(
    *,
    ...
    contains_pn68 = "SYNPN068" in selected_keys
...
def build_note_correction_scoped_copy_ready_prompt(package: Mapping[str, Any]) -> str:
    ...
    pn68_warning = (
        "SYNPN068 未匹配到 chunk，后续纠错审核需谨慎。本 scope 包含 SYNPN068 unmatched warning，SYNPN068 必须保留 alignment/unmatched 风险。"
        if "SYNPN068" in warning_keys or scope.get("pn68_in_scope")
        else "本 scope 不包含 SYNPN068 warning。"
    )
...
def _note_correction_sections(
    ...
    return [
        {
            **section,
            "has_pn68yptt": "SYNPN068" in (section.get("zotero_annotation_keys") or []),
        }
        ...
        batches.append(
            {
                ...
                "has_pn68yptt": any(item.get("zotero_annotation_key") == "SYNPN068" for item in items),
            }
        )
    return batches
...
def _pn68_scope_id(scopes: list[Mapping[str, Any]]) -> str | None:
    for scope in scopes:
        if scope.get("has_pn68yptt") or "SYNPN068" in (scope.get("zotero_annotation_keys") or []):
...
def _review_stats(items: list[Mapping[str, Any]], *, expected_count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "pn68yptt_present": any(item.get("zotero_annotation_key") == "SYNPN068" for item in items),
    }
...
def _validate_pn68_warning(items: list[Mapping[str, Any]], errors: list[str]) -> None:
    pn68 = next((item for item in items if item.get("zotero_annotation_key") == "SYNPN068"), None)
    if pn68 is None:
        errors.append("SYNPN068 item is required")
        return
    if not pn68.get("matched_expected_note_id"):
        errors.append("SYNPN068 must match a legal Search note identity")
    ...
    if status == "ok":
        errors.append("SYNPN068 cannot be marked ok because its original alignment is unmatched")
    if status not in {"unclear", "needs_revision"}:
        errors.append("SYNPN068 correction_status must be unclear or needs_revision")
    if not pn68.get("has_alignment_warning"):
        errors.append("SYNPN068 must preserve alignment_uncertain or unmatched risk")
    if issue_type != "alignment_uncertain" and not _has_alignment_warning(issue_type, reviewer_warning):
        errors.append("SYNPN068 issue_type or reviewer_warning must preserve alignment_uncertain or unmatched risk")
