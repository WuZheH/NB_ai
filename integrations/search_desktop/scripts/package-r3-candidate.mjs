...
if (await exists(candidateBase)) {
  throw new Error(`search_r3_candidate_already_exists:${candidateBase}`);
}
...
const manifest = {
  ...
  frontendSourceReusedFromR2: false,
};
await writeFile(
  join(packagedRoot, "r3-build-manifest.json"),
...
async function compareTrees(sourceRoot, packagedAssetRoot) {
  ...
  if (sourceFiles.join("\n") !== packagedFiles.join("\n")) {
    throw new Error("search_r3_frontend_asset_set_mismatch");
  }
  const rows = [];
  for (const path of sourceFiles) {
    ...
    if (sourceHash !== packagedHash) {
      throw new Error(`search_r3_frontend_asset_hash_mismatch:${path}`);
    }
...
function runNode(script, args = []) {
  return new Promise((resolvePromise, reject) => {
    ...
    child.once("exit", (code, signal) => {
      ...
      else reject(new Error(`search_r3_packaging_step_failed:${code ?? signal ?? "unknown"}`));
    });
  });
}
